...
def main(fraze: str):
    ...
    while True:
        ...
        if action == "1":
            ...
            ot = "всесчастливыесемьипохожидругнадругакаждаянесчастливаясемьянесчастливапосвоему"
            key = 28
            author = "Толстой Анна Каренина"

            print(f"Шифр-текст (ШТ): {fraze}")
            print(f"Расшифрованный текст (ОТ): {ot}")
            print(f"Ключ: {key}")
            print(f"Автор и произведение (ОТ): {author}")

            shifr_text = cipher.shifrover(author, key)
